...
impl<'a> ArrayI128<'a> {
    ...
    pub fn iter(&'a self) -> ArrayI128Iterator<'a> {
        ArrayI128Iterator {
    ...
    pub fn add_to_msg<const CAP: usize>(
        ...
        if let Some(var_info) = &self.variable_info {
            ...
            if let Some(scaler) = &self.scaling {
                let type_info = [0b0010_0101, 0b0001_1001, 0b0000_0000, 0b0000_0000];
                let quantization;
                let offset;
                ...
                let type_info: [u8; 4] = [0b0010_0101, 0b0000_1001, 0b0000_0000, 0b0000_0000];
                buf.try_extend_from_slice(&type_info)?;
            ...
            if let Some(scaler) = &self.scaling {
                let type_info: [u8; 4] = [0b0010_0101, 0b0001_0001, 0b0000_0000, 0b0000_0000];
                ...
                let type_info: [u8; 4] = [0b0010_0101, 0b0000_0001, 0b0000_0000, 0b0000_0000];
                buf.try_extend_from_slice(&type_info)?;
                buf.try_extend_from_slice(&number_of_dimensions)?;
                buf.try_extend_from_slice(self.dimensions.dimensions)?;
                buf.try_extend_from_slice(self.data)?;
...
#[cfg(feature = "serde")]
impl<'a> Serialize for ArrayI128<'a> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("ArrayI128", 4)?;
        state.serialize_field("variable_info", &self.variable_info)?;
        state.serialize_field("scaling", &self.scaling)?;

        let iter = ArrayItDimension::<i128> {
            is_big_endian: self.is_big_endian,
...
#[derive(Debug, Clone)]
pub struct ArrayI128Iterator<'a> {
    pub(crate) is_big_endian: bool,
    pub(crate) rest: &'a [u8],
}

impl Iterator for ArrayI128Iterator<'_> {
    type Item = i128;
...
impl ExactSizeIterator for ArrayI128Iterator<'_> {
    #[inline]
    fn len(&self) -> usize {
        self.rest.len() / 16
    }
...
impl<'a> IntoIterator for &'a ArrayI128<'a> {
    type Item = i128;
    type IntoIter = ArrayI128Iterator<'a>;
...
#[cfg(feature = "serde")]
impl<'a> Serialize for ArrayI128Iterator<'a> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
...
mod test {
    ...
    use crate::verbose::VerboseValue::ArrI128;
    use alloc::vec::Vec;
    use proptest::prelude::*;
    use std::format;
    use std::mem::size_of;

    type TestType<'a> = ArrayI128<'a>;
        ...
        fn write_read(ref name in "\\pc{0,20}", ref unit in "\\pc{0,20}", quantization in any::<f32>(), offset in any::<i128>(), dim_count in 0u16..5) {
            ...
            const BUFFER_SIZE: usize = 400;
               ...
               // test big endian with name & scaling
               {
                ...
                prop_assert_eq!(parsed_back, Ok((ArrI128(arr_u), &[] as &[u8])));
            }
            ...
            // test big endian with name without scaling
            {
                ...
                // Now wrap back
                let parsed_back = VerboseValue::from_slice(&msg_buff, is_big_endian);
                prop_assert_eq!(parsed_back, Ok((ArrI128(arr),&[] as &[u8])));
    ...
    fn two_values_bytes_for(is_big_endian: bool, value0: i128, value1: i128) -> [u8; 32 + 15] {
    ...
    proptest! {
        ...
        fn next(
            ...
            // empty
            {
                let mut iter = ArrayI128Iterator{
                    is_big_endian: false,
                    rest: &[],
                };
