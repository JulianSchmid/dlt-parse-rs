...
impl<'a> ArrayU64<'a> {
    ...
    pub fn iter(&'a self) -> ArrayU64Iterator<'a> {
        ArrayU64Iterator {
    ...
    pub fn add_to_msg<const CAP: usize>(
        ...
        if let Some(var_info) = &self.variable_info {
            ...
            if let Some(scaler) = &self.scaling {
                let type_info = [0b0100_0100, 0b0001_1001, 0b0000_0000, 0b0000_0000];
                let quantization;
                let offset;
                ...
                let type_info: [u8; 4] = [0b0100_0100, 0b0000_1001, 0b0000_0000, 0b0000_0000];
                buf.try_extend_from_slice(&type_info)?;
            ...
            if let Some(scaler) = &self.scaling {
                let type_info: [u8; 4] = [0b0100_0100, 0b0001_0001, 0b0000_0000, 0b0000_0000];
                ...
                let type_info: [u8; 4] = [0b0100_0100, 0b0000_0001, 0b0000_0000, 0b0000_0000];
                buf.try_extend_from_slice(&type_info)?;
                buf.try_extend_from_slice(&number_of_dimensions)?;
                buf.try_extend_from_slice(self.dimensions.dimensions)?;
                buf.try_extend_from_slice(self.data)?;
...
#[cfg(feature = "serde")]
impl<'a> Serialize for ArrayU64<'a> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("ArrayU64", 4)?;
        state.serialize_field("variable_info", &self.variable_info)?;
        state.serialize_field("scaling", &self.scaling)?;

        let iter = ArrayItDimension::<u64> {
...
#[derive(Debug, Clone)]
pub struct ArrayU64Iterator<'a> {
    pub(crate) is_big_endian: bool,
    pub(crate) rest: &'a [u8],
}

impl Iterator for ArrayU64Iterator<'_> {
    type Item = u64;
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        (self.rest.len() / 8, Some(self.rest.len() / 8))
    }
    ...
    fn last(self) -> Option<Self::Item> {
        if self.rest.len() < 8 {
            ...
            let last_index = ((self.rest.len() / 8) - 1) * 8;
            let bytes = unsafe {
                // SAFETY: Safe as len checked to be at least 8.
                [
    ...
    fn nth(&mut self, n: usize) -> Option<Self::Item> {
        ...
        //    n*8 <= self.rest.len() - 8
        //    n <= (self.rest.len() - 8) / 8
        if self.rest.len() >= 8 && n <= (self.rest.len() - 8) / 8 {
            let index = n * 8;
            let bytes = unsafe {
                [
                    // SAFETY: Safe as the length is checked beforehand to be at least n*8 + 8
            ...
            self.rest = unsafe {
                ...
                core::slice::from_raw_parts(
                    self.rest.as_ptr().add(index + 8),
                    self.rest.len() - index - 8,
                )
            };
            Some(result)
        } else {
...
impl ExactSizeIterator for ArrayU64Iterator<'_> {
    #[inline]
    fn len(&self) -> usize {
        self.rest.len() / 8
    }
...
impl<'a> IntoIterator for &'a ArrayU64<'a> {
    type Item = u64;
    type IntoIter = ArrayU64Iterator<'a>;
...
#[cfg(feature = "serde")]
impl<'a> Serialize for ArrayU64Iterator<'a> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
...
mod test {
    ...
    use crate::verbose::VerboseValue::ArrU64;
    use alloc::vec::Vec;
    use proptest::prelude::*;
    use std::format;
    use std::mem::size_of;

    type TestType<'a> = ArrayU64<'a>;
    type InternalTypes = u64;

    proptest! {
        #[test]
        fn write_read(ref name in "\\pc{0,20}", ref unit in "\\pc{0,20}", quantization in any::<f32>(), offset in any::<i64>(), dim_count in 0u16..5) {
            const TYPE_INFO_RAW: [u8; 4] = [0b0100_0100, 0b0000_0001, 0b0000_0000, 0b0000_0000];
            ...
            const BUFFER_SIZE: usize = 400;
               ...
               // test big endian with name & scaling
               {
                ...
                prop_assert_eq!(parsed_back, Ok((ArrU64(arr_u), &[] as &[u8])));
            }
            ...
            // test big endian with name without scaling
            {
                ...
                // Now wrap back
                let parsed_back = VerboseValue::from_slice(&msg_buff, is_big_endian);
                prop_assert_eq!(parsed_back, Ok((ArrU64(arr),&[] as &[u8])));
    ...
    proptest! {
        #[test]
        fn data(ref data in "\\pc{0,100}", ref dimensions in "\\pc{0,100}", quantization in any::<f32>(), offset in any::<i64>()) {

            let arr_dim = ArrayDimensions {dimensions: dimensions.as_bytes(), is_big_endian: true };
            let scaling = Some(Scaling { quantization, offset });
            let arr = TestType {is_big_endian: true, dimensions:arr_dim,variable_info:None,data:data.as_bytes(), scaling };
        ...
        fn iterator(dim_count in 0u16..5) {
            ...
            // test big endian without name & scaling
            {
                ...
                for item in arr.iter() {
                    prop_assert_eq!(item, InternalTypes::from_be_bytes([content[cnt], content[cnt+1], content[cnt+2], content[cnt+3], content[cnt+4], content[cnt+5], content[cnt+6], content[cnt+7]]));
    ...
    fn two_values_bytes_for(is_big_endian: bool, value0: u64, value1: u64) -> [u8; 16 + 7] {
