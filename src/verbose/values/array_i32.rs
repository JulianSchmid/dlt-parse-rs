...
impl<'a> ArrayI32<'a> {
    ...
    pub fn iter(&'a self) -> ArrayI32Iterator<'a> {
        ArrayI32Iterator {
    ...
    pub fn add_to_msg<const CAP: usize>(
        ...
        if let Some(var_info) = &self.variable_info {
            ...
            if let Some(scaler) = &self.scaling {
                let type_info = [0b0010_0011, 0b0001_1001, 0b0000_0000, 0b0000_0000];
                let quantization;
                let offset;
                ...
                let type_info: [u8; 4] = [0b0010_0011, 0b0000_1001, 0b0000_0000, 0b0000_0000];
                buf.try_extend_from_slice(&type_info)?;
            ...
            if let Some(scaler) = &self.scaling {
                let type_info: [u8; 4] = [0b0010_0011, 0b0001_0001, 0b0000_0000, 0b0000_0000];
                ...
                let type_info: [u8; 4] = [0b0010_0011, 0b0000_0001, 0b0000_0000, 0b0000_0000];
                buf.try_extend_from_slice(&type_info)?;
                buf.try_extend_from_slice(&number_of_dimensions)?;
                buf.try_extend_from_slice(self.dimensions.dimensions)?;
                buf.try_extend_from_slice(self.data)?;
...
#[cfg(feature = "serde")]
impl<'a> Serialize for ArrayI32<'a> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("ArrayI32", 4)?;
        state.serialize_field("variable_info", &self.variable_info)?;
        state.serialize_field("scaling", &self.scaling)?;

        let iter = ArrayItDimension::<i32> {
            is_big_endian: self.is_big_endian,
...
#[derive(Debug, Clone)]
pub struct ArrayI32Iterator<'a> {
    pub(crate) is_big_endian: bool,
    pub(crate) rest: &'a [u8],
}

impl Iterator for ArrayI32Iterator<'_> {
    type Item = i32;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.rest.len() < 4 {
            ...
            let result = if self.is_big_endian {
                i32::from_be_bytes([self.rest[0], self.rest[1], self.rest[2], self.rest[3]])
            } else {
                i32::from_le_bytes([self.rest[0], self.rest[1], self.rest[2], self.rest[3]])
            };
...
impl ExactSizeIterator for ArrayI32Iterator<'_> {
    #[inline]
    fn len(&self) -> usize {
        self.rest.len() / 4
    }
...
impl<'a> IntoIterator for &'a ArrayI32<'a> {
    type Item = i32;
    type IntoIter = ArrayI32Iterator<'a>;
...
#[cfg(feature = "serde")]
impl<'a> Serialize for ArrayI32Iterator<'a> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
...
mod test {
    ...
    use crate::verbose::VerboseValue::ArrI32;
    use alloc::vec::Vec;
    use proptest::prelude::*;
    use std::format;
    use std::mem::size_of;

    type TestType<'a> = ArrayI32<'a>;
    type InternalTypes = i32;

    proptest! {
        #[test]
        fn write_read(ref name in "\\pc{0,20}", ref unit in "\\pc{0,20}", quantization in any::<f32>(), offset in any::<i32>(), dim_count in 0u16..5) {
            const TYPE_INFO_RAW: [u8; 4] = [0b0010_0011, 0b0000_0001, 0b0000_0000, 0b0000_0000];
            ...
            const BUFFER_SIZE: usize = 400;
               ...
               // test big endian with name & scaling
               {
                ...
                prop_assert_eq!(parsed_back, Ok((ArrI32(arr_u), &[] as &[u8])));
            }
            ...
            // test big endian with name without scaling
            {
                ...
                // Now wrap back
                let parsed_back = VerboseValue::from_slice(&msg_buff, is_big_endian);
                prop_assert_eq!(parsed_back, Ok((ArrI32(arr),&[] as &[u8])));
        ...
        fn iterator(dim_count in 0u16..5) {
            ...
            // test big endian without name & scaling
            {
                ...
                for item in arr.iter() {
                    prop_assert_eq!(item, InternalTypes::from_be_bytes([content[cnt], content[cnt+1], content[cnt+2], content[cnt+3]]));
    ...
    fn two_values_bytes_for(is_big_endian: bool, value0: i32, value1: i32) -> [u8; 8 + 3] {
