...
impl<'a> ArrayU32<'a> {
    ...
    pub fn iter(&'a self) -> ArrayU32Iterator<'a> {
        ArrayU32Iterator {
    ...
    pub fn add_to_msg<const CAP: usize>(
        ...
        if let Some(var_info) = &self.variable_info {
            ...
            if let Some(scaler) = &self.scaling {
                let type_info = [0b0100_0011, 0b0001_1001, 0b0000_0000, 0b0000_0000];
                let quantization;
                let offset: [u8; 4];
                ...
                let type_info: [u8; 4] = [0b0100_0011, 0b0000_1001, 0b0000_0000, 0b0000_0000];
                buf.try_extend_from_slice(&type_info)?;
            ...
            if let Some(scaler) = &self.scaling {
                let type_info: [u8; 4] = [0b0100_0011, 0b0001_0001, 0b0000_0000, 0b0000_0000];
                ...
                let type_info: [u8; 4] = [0b0100_0011, 0b0000_0001, 0b0000_0000, 0b0000_0000];
                buf.try_extend_from_slice(&type_info)?;
                buf.try_extend_from_slice(&number_of_dimensions)?;
                buf.try_extend_from_slice(self.dimensions.dimensions)?;
                buf.try_extend_from_slice(self.data)?;
...
#[cfg(feature = "serde")]
impl<'a> Serialize for ArrayU32<'a> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("ArrayU32", 4)?;
        state.serialize_field("variable_info", &self.variable_info)?;
        state.serialize_field("scaling", &self.scaling)?;

        let iter = ArrayItDimension::<u32> {
...
#[derive(Debug, Clone)]
pub struct ArrayU32Iterator<'a> {
    pub(crate) is_big_endian: bool,
    pub(crate) rest: &'a [u8],
}

impl Iterator for ArrayU32Iterator<'_> {
    type Item = u32;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.rest.len() < 4 {
            ...
            let result = if self.is_big_endian {
                u32::from_be_bytes([self.rest[0], self.rest[1], self.rest[2], self.rest[3]])
            } else {
                u32::from_le_bytes([self.rest[0], self.rest[1], self.rest[2], self.rest[3]])
            };
...
impl<'a> IntoIterator for &'a ArrayU32<'a> {
    type Item = u32;
    type IntoIter = ArrayU32Iterator<'a>;
...
#[cfg(feature = "serde")]
impl<'a> Serialize for ArrayU32Iterator<'a> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
...
mod test {
    ...
    use crate::verbose::VerboseValue::ArrU32;
    use alloc::vec::Vec;
    use proptest::prelude::*;
    use std::format;
    use std::mem::size_of;

    type TestType<'a> = ArrayU32<'a>;
    type InternalTypes = u32;

    proptest! {
        #[test]
        fn write_read(ref name in "\\pc{0,20}", ref unit in "\\pc{0,20}", quantization in any::<f32>(), offset in any::<i32>(), dim_count in 0u16..5) {
            const TYPE_INFO_RAW: [u8; 4] = [0b0100_0011, 0b0000_0001, 0b0000_0000, 0b0000_0000];
            ...
            const BUFFER_SIZE: usize = 400;
               ...
               // test big endian with name & scaling
               {
                ...
                prop_assert_eq!(parsed_back, Ok((ArrU32(arr_u), &[] as &[u8])));
            }
            ...
            // test big endian with name without scaling
            {
                ...
                // Now wrap back
                let parsed_back = VerboseValue::from_slice(&msg_buff, is_big_endian);
                prop_assert_eq!(parsed_back, Ok((ArrU32(arr),&[] as &[u8])));
            ...
            // test little endian with scaling and without name
            {
             ...
             // capacity error big endian with name, without scaling
             {
                ...
                for i in 0..DIM_COUNT as u32 {
                        dimensions.extend_from_slice(&(1 as u32).to_be_bytes());
