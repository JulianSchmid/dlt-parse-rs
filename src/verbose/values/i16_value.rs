    ...
    pub fn add_to_msg<const CAP: usize>(
        ...
        if let Some(var_info) = &self.variable_info {
            ...
            if let Some(scaler) = &self.scaling {
                let type_info = [0b0010_0010, 0b0001_1000, 0b0000_0000, 0b0000_0000];
                let quantization;
                let offset: [u8; 4];
                ...
                let type_info = [0b0010_0010, 0b0000_1000, 0b0000_0000, 0b0000_0000];
        ...
        // No name & unit
            ...
            let type_info: [u8; 4] = [0b0010_0010, 0b0001_0000, 0b0000_0000, 0b0000_0000];
            ...
            let type_info: [u8; 4] = [0b0010_0010, 0b0000_0000, 0b0000_0000, 0b0000_0000];
            buf.try_extend_from_slice(&type_info)?;
        }
...
mod test {
    ...
    use crate::verbose::VerboseValue::I16;
    use alloc::vec::Vec;
    ...
    proptest! {
        #[test]
        fn write_read(value in any::<i16>(), ref name in "\\pc{0,20}", ref unit in "\\pc{0,20}", quantization in any::<f32>(), offset in any::<i32>()) {
            ...
            // test big endian with name and fixed point
            {
                ...
                let i16_value = I16Value {variable_info, scaling, value};
                ...
                prop_assert_eq!(i16_value.add_to_msg(&mut msg_buff, is_big_endian), Ok(()));

                content_buff.extend_from_slice(&[0b0010_0010, 0b0001_1000, 0b0000_0000, 0b0000_0000, len_name_be[0], len_name_be[1], len_unit_be[0], len_unit_be[1]]);
                ...
                prop_assert_eq!(parsed_back, Ok((I16(i16_value),&[] as &[u8])));
            ...
            // test little endian with name and fixed point
            {
                ...
                content_buff.extend_from_slice(&[0b0010_0010, 0b0001_1000, 0b0000_0000, 0b0000_0000, len_name_le[0], len_name_le[1], len_unit_le[0], len_unit_le[1]]);
                content_buff.extend_from_slice(name.as_bytes());
                content_buff.push(0);
                content_buff.extend_from_slice(unit.as_bytes());
                content_buff.push(0);
            ...
            // test big endian with name without fixed point
          {
            ...
            content_buff.extend_from_slice(&[0b0010_0010, 0b0000_1000, 0b0000_0000, 0b0000_0000, len_name_be[0], len_name_be[1], len_unit_be[0], len_unit_be[1]]);
            content_buff.extend_from_slice(name.as_bytes());
            content_buff.push(0);
            content_buff.extend_from_slice(unit.as_bytes());
            content_buff.push(0);
            ...
            // test little endian with name without fixed point
            {
                ...
                content_buff.extend_from_slice(&[0b0010_0010, 0b0000_1000, 0b0000_0000, 0b0000_0000, len_name_le[0], len_name_le[1], len_unit_le[0], len_unit_le[1]]);
                content_buff.extend_from_slice(name.as_bytes());
                content_buff.push(0);
                content_buff.extend_from_slice(unit.as_bytes());
                content_buff.push(0);
        ...
        // test big endian without name, unit and without fixed point
        {
            ...
            content_buff.extend_from_slice(&[0b0010_0010, 0b0000_0000, 0b0000_0000, 0b0000_0000]);
            content_buff.extend_from_slice(&value.to_be_bytes());
        ...
        // test big endian with fixed point and without name
        {
            ...
            content_buff.extend_from_slice(&[0b0010_0010, 0b0001_0000, 0b0000_0000, 0b0000_0000]);
            content_buff.extend_from_slice(&quantization.to_be_bytes());
            content_buff.extend_from_slice(&offset.to_be_bytes());
            content_buff.extend_from_slice(&value.to_be_bytes());
        ...
        // test little endian with fixed point and without name
        {
         ...
         // Capacity error big endian with name and scaling
         {
            ...
            prop_assert_eq!(i16_value.add_to_msg(&mut msg_buff, is_big_endian), Err(CapacityError::new(())));
    ...
    proptest! {
        ...
        fn debug(value in any::<i16>(), ref name in "\\pc{0,20}", ref unit in "\\pc{0,20}", quantization in any::<f32>(), offset in any::<i32>()) {
            ...
            let value_struct: I16Value = I16Value {variable_info: Some(VariableInfoUnit { name, unit }), scaling: Some(Scaling { quantization, offset }), value};
            assert_eq!(
                    format!(
                        "I16Value {{ variable_info: {:?}, scaling: {:?}, value: {:?} }}",
            ...
            { // Test with name, unit & without scaling
                let value_struct: I16Value = I16Value {variable_info: Some(VariableInfoUnit { name, unit }), scaling: None, value};
                assert_eq!(
                        format!(
                            "I16Value {{ variable_info: {:?}, scaling: {:?}, value: {:?} }}",
                            value_struct.variable_info,
            ...
            { // Test without name, unit & without scaling
                let value_struct: I16Value = I16Value {variable_info: None, scaling: None, value};
                assert_eq!(
                        format!(
                            "I16Value {{ variable_info: {:?}, scaling: {:?}, value: {:?} }}",
                            value_struct.variable_info,
           ...
           { // Test with scaling, but without name & unit
            let value_struct: I16Value = I16Value {variable_info: None, scaling: Some(Scaling { quantization, offset }), value};
            assert_eq!(
                    format!(
                        "I16Value {{ variable_info: {:?}, scaling: {:?}, value: {:?} }}",
                        value_struct.variable_info,
