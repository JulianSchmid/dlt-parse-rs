...
impl<'a> F16Value<'a> {
    ...
    pub fn add_to_msg<const CAP: usize>(
        ...
        if let Some(var_info) = &self.variable_info {
            ...
            let type_info = [0b1000_0010, 0b0000_0000, 0b0000_0000, 0b0000_0000];
            buf.try_extend_from_slice(&type_info)?;
        }
...
mod test {
    ...
    use crate::verbose::VerboseValue::F16;
    use alloc::vec::Vec;
    ...
    proptest! {
        #[test]
        fn write_read(value in any::<u16>(), ref name in "\\pc{0,20}", ref unit in "\\pc{0,20}") {
            ...
            // test big endian with name
            {
                ...
                content_buff.extend_from_slice(&[0b1000_0010, 0b0000_1000, 0b0000_0000, 0b0000_0000, len_name_be[0], len_name_be[1], len_unit_be[0], len_unit_be[1]]);
                ...
                prop_assert_eq!(parsed_back, Ok((F16(f16_value),&[] as &[u8])));
            }
            ...
            // test little endian with name
            {
                ...
                content_buff.extend_from_slice(&[0b1000_0010, 0b0000_1000, 0b0000_0000, 0b0000_0000, len_name_le[0], len_name_le[1], len_unit_le[0], len_unit_le[1]]);
                content_buff.extend_from_slice(name.as_bytes());
                content_buff.push(0);
                content_buff.extend_from_slice(unit.as_bytes());
                content_buff.push(0);
            ...
            // test big endian without name
            {
                ...
                content_buff.extend_from_slice(&[0b1000_0010, 0b0000_0000, 0b0000_0000, 0b0000_0000]);
                content_buff.extend_from_slice(&value.to_be_bytes());
