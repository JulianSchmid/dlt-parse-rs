    ...
    pub fn add_to_msg<const CAP: usize>(
        ...
        if let Some(var_info) = &self.variable_info {
            ...
            if let Some(scaler) = &self.scaling {
                let type_info = [0b0100_0001, 0b0001_1000, 0b0000_0000, 0b0000_0000];
                let quantization;
                let offset: [u8; 4];
                ...
                let type_info = [0b0100_0001, 0b0000_1000, 0b0000_0000, 0b0000_0000];
        ...
        // No name & unit
            ...
            if let Some(scaler) = &self.scaling {
                let type_info: [u8; 4] = [0b0100_0001, 0b0001_0000, 0b0000_0000, 0b0000_0000];
                ...
                let type_info: [u8; 4] = [0b0100_0001, 0b0000_0000, 0b0000_0000, 0b0000_0000];
                buf.try_extend_from_slice(&type_info)?;
                if buf.remaining_capacity() > 0 {
                    unsafe {
                        buf.push_unchecked(self.value as u8);
...
mod test {
    ...
    use crate::verbose::VerboseValue::U8;
    use alloc::vec::Vec;
    ...
    proptest! {
        #[test]
        fn write_read(value in any::<u8>(), ref name in "\\pc{0,20}", ref unit in "\\pc{0,20}", quantization in any::<f32>(), offset in any::<i32>()) {

            // auto_gen_positive_tests!([0b0100_0001, 0b0000_0000, 0b0000_0000, 0b0000_0000], U8Value, U8, value, name, unit, quantization, offset);
            ...
            // test big endian with name and fixed point
            {
                ...
                let u8_value = U8Value {variable_info, scaling, value};
                ...
                prop_assert_eq!(u8_value.add_to_msg(&mut msg_buff, is_big_endian), Ok(()));

                content_buff.extend_from_slice(&[0b0100_0001, 0b0001_1000, 0b0000_0000, 0b0000_0000, len_name_be[0], len_name_be[1], len_unit_be[0], len_unit_be[1]]);
                ...
                prop_assert_eq!(parsed_back, Ok((U8(u8_value),&[] as &[u8])));
            ...
            // test little endian with name and fixed point
            {
                ...
                content_buff.extend_from_slice(&[0b0100_0001, 0b0001_1000, 0b0000_0000, 0b0000_0000, len_name_le[0], len_name_le[1], len_unit_le[0], len_unit_le[1]]);
                content_buff.extend_from_slice(name.as_bytes());
                content_buff.push(0);
                content_buff.extend_from_slice(unit.as_bytes());
                content_buff.push(0);
          ...
          // test big endian with name without fixed point
          {
            ...
            content_buff.extend_from_slice(&[0b0100_0001, 0b0000_1000, 0b0000_0000, 0b0000_0000, len_name_be[0], len_name_be[1], len_unit_be[0], len_unit_be[1]]);
            content_buff.extend_from_slice(name.as_bytes());
            content_buff.push(0);
            content_buff.extend_from_slice(unit.as_bytes());
            content_buff.push(0);
            ...
            prop_assert_eq!(parsed_back, Ok((U8(u8_value),&[] as &[u8])));
                  ...
                  // test little endian with name without fixed point
                  {
                    ...
                    content_buff.extend_from_slice(&[0b0100_0001, 0b0000_1000, 0b0000_0000, 0b0000_0000, len_name_le[0], len_name_le[1], len_unit_le[0], len_unit_le[1]]);
                    content_buff.extend_from_slice(name.as_bytes());
                    content_buff.push(0);
                    content_buff.extend_from_slice(unit.as_bytes());
                    content_buff.push(0);
          ...
          // test big endian without name, unit and without fixed point
          {
            ...
            content_buff.extend_from_slice(&[0b0100_0001, 0b0000_0000, 0b0000_0000, 0b0000_0000]);
            content_buff.push(value as u8);
          ...
          // test little endian without name, unit and without fixed point
          {
            ...
            // test big endian with fixed point and without name
            {
                ...
                content_buff.extend_from_slice(&[0b0100_0001, 0b0001_0000, 0b0000_0000, 0b0000_0000]);
                content_buff.extend_from_slice(&quantization.to_be_bytes());
                content_buff.extend_from_slice(&offset.to_be_bytes());
                content_buff.push(value as u8);
            ...
            // Capacity error big endian with name and scaling
            {
                ...
                prop_assert_eq!(u8_value.add_to_msg(&mut msg_buff, is_big_endian), Err(CapacityError::new(())));
    ...
    proptest! {
        #[test]
        fn debug(value in any::<u8>(), ref name in "\\pc{0,20}", ref unit in "\\pc{0,20}", quantization in any::<f32>(), offset in any::<i32>()) {
            { // Test with name, unit & scaling
            let value_struct: U8Value = U8Value {variable_info: Some(VariableInfoUnit { name, unit }), scaling: Some(Scaling { quantization, offset }), value};
            assert_eq!(
                    format!(
                        "U8Value {{ variable_info: {:?}, scaling: {:?}, value: {:?} }}",
            ...
            { // Test with name, unit & without scaling
                let value_struct: U8Value = U8Value {variable_info: Some(VariableInfoUnit { name, unit }), scaling: None, value};
                assert_eq!(
                        format!(
                            "U8Value {{ variable_info: {:?}, scaling: {:?}, value: {:?} }}",
                            value_struct.variable_info,
            ...
            { // Test without name, unit & without scaling
                let value_struct: U8Value = U8Value {variable_info: None, scaling: None, value};
                assert_eq!(
                        format!(
                            "U8Value {{ variable_info: {:?}, scaling: {:?}, value: {:?} }}",
                            value_struct.variable_info,
