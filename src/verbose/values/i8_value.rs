    ...
    pub fn add_to_msg<const CAP: usize>(
        ...
        if let Some(var_info) = &self.variable_info {
            ...
            if let Some(scaler) = &self.scaling {
                let type_info = [0b0010_0001, 0b0001_1000, 0b0000_0000, 0b0000_0000];
                let quantization;
                let offset: [u8; 4];
                ...
                let type_info = [0b0010_0001, 0b0000_1000, 0b0000_0000, 0b0000_0000];
            ...
            if buf.remaining_capacity() > 0 {
                unsafe {
                    buf.push_unchecked(self.value as u8);
                }
                Ok(())
            } else {
        ...
        // No name & unit
            ...
            let type_info: [u8; 4] = [0b0010_0001, 0b0001_0000, 0b0000_0000, 0b0000_0000];
            ...
            let type_info: [u8; 4] = [0b0010_0001, 0b0000_0000, 0b0000_0000, 0b0000_0000];
            buf.try_extend_from_slice(&type_info)?;
            if buf.remaining_capacity() > 0 {
                unsafe {
                    buf.push_unchecked(self.value as u8);
...
mod test {
    ...
    proptest! {
        #[test]
        fn write_read(value in any::<i8>(), ref name in "\\pc{0,20}", ref unit in "\\pc{0,20}", quantization in any::<f32>(), offset in any::<i32>()) {
            const MAX_SYMBOL_LENGTH_NAME: usize = 20;
            const MAX_SYMBOL_LENGTH_UNIT: usize = 20;
            const FIXED_POINT_LENGTH: usize = 8;
            const BYTES_NEEDED: usize = 5;
            const BYTES_NEEDED_WITH_NAME: usize = 6 + BYTES_NEEDED;

            // The buffer needs to be sized the (max len of the name + max len unit) * 4 + 11 Byte. (11 Byte: 4 Byte TypeInfo + 2 Bytes Length of Name + 1 Byte Null Terminator of Name + 2 Byte unit Length + 1 Byte Null Terminator Unit + 1 Byte Data)
            ...
            // test big endian with name and fixed point
            {
                ...
                let i8_value = I8Value {variable_info, scaling, value};
                ...
                prop_assert_eq!(i8_value.add_to_msg(&mut msg_buff, is_big_endian), Ok(()));

                content_buff.extend_from_slice(&[0b0010_0001, 0b0001_1000, 0b0000_0000, 0b0000_0000, len_name_be[0], len_name_be[1], len_unit_be[0], len_unit_be[1]]);
            ...
            // test little endian with name and fixed point
            {
                ...
                content_buff.extend_from_slice(&[0b0010_0001, 0b0001_1000, 0b0000_0000, 0b0000_0000, len_name_le[0], len_name_le[1], len_unit_le[0], len_unit_le[1]]);
                content_buff.extend_from_slice(name.as_bytes());
                content_buff.push(0);
                content_buff.extend_from_slice(unit.as_bytes());
                content_buff.push(0);
          ...
          // test big endian with name without fixed point
          {
            ...
            content_buff.extend_from_slice(&[0b0010_0001, 0b0000_1000, 0b0000_0000, 0b0000_0000, len_name_be[0], len_name_be[1], len_unit_be[0], len_unit_be[1]]);
            content_buff.extend_from_slice(name.as_bytes());
            content_buff.push(0);
            content_buff.extend_from_slice(unit.as_bytes());
            content_buff.push(0);
            ...
            prop_assert_eq!(parsed_back, Ok((I8(i8_value),&[] as &[u8])));
                  ...
                  // test little endian with name without fixed point
                  {
                    ...
                    content_buff.extend_from_slice(&[0b0010_0001, 0b0000_1000, 0b0000_0000, 0b0000_0000, len_name_le[0], len_name_le[1], len_unit_le[0], len_unit_le[1]]);
                    content_buff.extend_from_slice(name.as_bytes());
                    content_buff.push(0);
                    content_buff.extend_from_slice(unit.as_bytes());
                    content_buff.push(0);
          ...
          // test big endian without name, unit and without fixed point
          {
            ...
            content_buff.extend_from_slice(&[0b0010_0001, 0b0000_0000, 0b0000_0000, 0b0000_0000]);
            content_buff.push(value as u8);
          ...
          // test little endian without name, unit and without fixed point
          {
            ...
            // test big endian with fixed point and without name
            {
                ...
                content_buff.extend_from_slice(&[0b0010_0001, 0b0001_0000, 0b0000_0000, 0b0000_0000]);
                content_buff.extend_from_slice(&quantization.to_be_bytes());
                content_buff.extend_from_slice(&offset.to_be_bytes());
                content_buff.push(value as u8);
            ...
            // Capacity error big endian with name and scaling
            {
                ...
                prop_assert_eq!(i8_value.add_to_msg(&mut msg_buff, is_big_endian), Err(CapacityError::new(())));
            ...
            {
                const SLICE_LEN: usize = FIXED_POINT_LENGTH + BYTES_NEEDED_WITH_NAME - 1;   //min length of name and unit 1, because of null temrinator
                let variable_info = Some(VariableInfoUnit { name , unit });
                let scaling = Some(Scaling { quantization, offset });
            ...
            {
                const SLICE_LEN: usize = BYTES_NEEDED_WITH_NAME - 1;   //min length of name and unit 1, because of null temrinator
                let variable_info = Some(VariableInfoUnit { name , unit });
                let scaling = None;
            ...
            {
                const SLICE_LEN: usize = BYTES_NEEDED - 1;   //min length of name and unit 1, because of null temrinator
                let variable_info = None;
                let scaling = None;
            ...
            {
                const SLICE_LEN: usize = FIXED_POINT_LENGTH + BYTES_NEEDED - 1;   //min length of name and unit 1, because of null temrinator
                let variable_info = None;
                let scaling = Some(Scaling { quantization, offset });
            ...
            {
                const SLICE_LEN: usize = BYTES_NEEDED + FIXED_POINT_LENGTH - 2;
                let variable_info = None;
                let scaling = Some(Scaling { quantization, offset });
            ...
            {
                const SLICE_LEN: usize = BYTES_NEEDED;
                let variable_info = None;
                let scaling = Some(Scaling { quantization, offset });
    ...
    proptest! {
        #[test]
        fn debug(value in any::<i8>(), ref name in "\\pc{0,20}", ref unit in "\\pc{0,20}", quantization in any::<f32>(), offset in any::<i32>()) {
            { // Test with name, unit & scaling
            let value_struct: I8Value = I8Value {variable_info: Some(VariableInfoUnit { name, unit }), scaling: Some(Scaling { quantization, offset }), value};
            assert_eq!(
                    format!(
                        "I8Value {{ variable_info: {:?}, scaling: {:?}, value: {:?} }}",
            ...
            { // Test with name, unit & without scaling
                let value_struct: I8Value = I8Value {variable_info: Some(VariableInfoUnit { name, unit }), scaling: None, value};
                assert_eq!(
                        format!(
                            "I8Value {{ variable_info: {:?}, scaling: {:?}, value: {:?} }}",
                            value_struct.variable_info,
            ...
            { // Test without name, unit & without scaling
                let value_struct: I8Value = I8Value {variable_info: None, scaling: None, value};
                assert_eq!(
                        format!(
                            "I8Value {{ variable_info: {:?}, scaling: {:?}, value: {:?} }}",
                            value_struct.variable_info,
           ...
           { // Test with scaling, but without name & unit
            let value_struct: I8Value = I8Value {variable_info: None, scaling: Some(Scaling { quantization, offset }), value};
            assert_eq!(
                    format!(
                        "I8Value {{ variable_info: {:?}, scaling: {:?}, value: {:?} }}",
                        value_struct.variable_info,
    ...
    proptest! {
        #[test]
        fn clone(value in any::<i8>(), ref name in "\\pc{0,20}", ref unit in "\\pc{0,20}", quantization in any::<f32>(), offset in any::<i32>()) {
            { // Test with name, unit & scaling
            let value_struct: I8Value = I8Value {variable_info: Some(VariableInfoUnit { name, unit }), scaling: Some(Scaling { quantization, offset }), value};
            let cloned = value_struct.clone();
            assert_eq!(cloned, value_struct);
            }
