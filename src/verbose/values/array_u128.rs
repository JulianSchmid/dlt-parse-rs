...
impl<'a> ArrayU128<'a> {
    ...
    pub fn iter(&'a self) -> ArrayU128Iterator<'a> {
        ArrayU128Iterator {
    ...
    pub fn add_to_msg<const CAP: usize>(
        ...
        if let Some(var_info) = &self.variable_info {
            ...
            if let Some(scaler) = &self.scaling {
                let type_info = [0b0100_0101, 0b0001_1001, 0b0000_0000, 0b0000_0000];
                let quantization;
                let offset;
                ...
                let type_info: [u8; 4] = [0b0100_0101, 0b0000_1001, 0b0000_0000, 0b0000_0000];
                buf.try_extend_from_slice(&type_info)?;
            ...
            if let Some(scaler) = &self.scaling {
                let type_info: [u8; 4] = [0b0100_0101, 0b0001_0001, 0b0000_0000, 0b0000_0000];
                ...
                let type_info: [u8; 4] = [0b0100_0101, 0b0000_0001, 0b0000_0000, 0b0000_0000];
                buf.try_extend_from_slice(&type_info)?;
                buf.try_extend_from_slice(&number_of_dimensions)?;
                buf.try_extend_from_slice(self.dimensions.dimensions)?;
                buf.try_extend_from_slice(self.data)?;
...
#[cfg(feature = "serde")]
impl<'a> Serialize for ArrayU128<'a> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("ArrayU128", 4)?;
        state.serialize_field("variable_info", &self.variable_info)?;
        state.serialize_field("scaling", &self.scaling)?;

        let iter = ArrayItDimension::<u128> {
...
#[derive(Debug, Clone)]
pub struct ArrayU128Iterator<'a> {
    pub(crate) is_big_endian: bool,
    pub(crate) rest: &'a [u8],
}

impl Iterator for ArrayU128Iterator<'_> {
    type Item = u128;
...
impl ExactSizeIterator for ArrayU128Iterator<'_> {
    #[inline]
    fn len(&self) -> usize {
        self.rest.len() / 16
    }
...
impl<'a> IntoIterator for &'a ArrayU128<'a> {
    type Item = u128;
    type IntoIter = ArrayU128Iterator<'a>;
...
#[cfg(feature = "serde")]
impl<'a> Serialize for ArrayU128Iterator<'a> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
...
mod test {
    ...
    use crate::verbose::VerboseValue::ArrU128;
    use alloc::vec::Vec;
    use proptest::prelude::*;
    use std::format;
    use std::mem::size_of;

    type TestType<'a> = ArrayU128<'a>;
    type InternalTypes = u128;

    proptest! {
        #[test]
        fn write_read(ref name in "\\pc{0,20}", ref unit in "\\pc{0,20}", quantization in any::<f32>(), offset in any::<i128>(), dim_count in 0u16..5) {
            const TYPE_INFO_RAW: [u8; 4] = [0b0100_0101, 0b0000_0001, 0b0000_0000, 0b0000_0000];
            ...
            const BUFFER_SIZE: usize = 400;
               ...
               // test big endian with name & scaling
               {
                ...
                prop_assert_eq!(parsed_back, Ok((ArrU128(arr_u), &[] as &[u8])));
            }
            ...
            // test big endian with name without scaling
            {
                ...
                // Now wrap back
                let parsed_back = VerboseValue::from_slice(&msg_buff, is_big_endian);
                prop_assert_eq!(parsed_back, Ok((ArrU128(arr),&[] as &[u8])));
    ...
    proptest! {
        #[test]
        fn data(ref data in "\\pc{0,100}", ref dimensions in "\\pc{0,100}", quantization in any::<f32>(), offset in any::<i128>()) {
        ...
        fn iterator(dim_count in 0u16..5) {
            ...
            // test big endian without name & scaling
            {
                ...
                for item in arr.iter() {
                    prop_assert_eq!(item, InternalTypes::from_be_bytes([content[cnt], content[cnt+1], content[cnt+2], content[cnt+3], content[cnt+4], content[cnt+5], content[cnt+6], content[cnt+7], content[cnt+8], content[cnt+9], content[cnt+10], content[cnt+11], content[cnt+12], content[cnt+13], content[cnt+14], content[cnt+15]]));
    ...
    fn two_values_bytes_for(is_big_endian: bool, value0: u128, value1: u128) -> [u8; 32 + 15] {
    ...
    proptest! {
        ...
        fn next(
            ...
            // empty
            {
                let mut iter = ArrayU128Iterator{
                    is_big_endian: false,
                    rest: &[],
                };
