/// Describing the range of received data in an DLT file transfer stream.
#[derive(Debug, Copy, Clone, Eq, Hash, PartialEq, Ord, PartialOrd)]
pub struct DltFtRange {
    /// Offset of section
    pub start: u64,
...
impl DltFtRange {
    ...
    pub fn merge(&self, other: DltFtRange) -> Option<DltFtRange> {
...
mod test {
    ...
    fn debug_clone_eq() {
        use alloc::format;
        let section = DltFtRange { start: 1, end: 2 };
    ...
    fn is_value_connected() {
        let s = DltFtRange { start: 5, end: 9 };
    ...
    fn merge() {
        ...
        for t in tests {
            ...
            let expected = t.2.map(|v| DltFtRange {
                start: v.0,
                end: v.1,
            });
